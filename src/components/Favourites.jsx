...
function Favourites() {
  ...
  const onNext = () => indexOfLastMovie < filteredMovies.length && setCurrentPage(currentPage+1);
    ...
    <>
      <div className="mt-6 flex space-x-2 justify-center">
        {genres.map((genre, idx) => (
          <button key={idx} className={`py-1 px-2 rounded-lg font-bold text-lg text-white ${currGenre===genre?"bg-blue-600":"bg-gray-400 hover:bg-blue-400"}`} 
            ...
            {currentMovies.map(m => (
              <tr key={m.id} className="hover:bg-gray-50">
                <th className="flex items-center px-6 py-4 font-normal text-gray-900 space-x-2">
                  <img className="h-[6rem] w-[10rem] object-fit" src={`https://image.tmdb.org/t/p/original/${m.poster_path}`} alt={m.title} />
                  <div className="font-medium text-gray-700 text-sm">{m.title}</div>
                </th>
                <td className="px-6 pl-12 py-4">{m.vote_average.toFixed(2)}</td>
                <td className="px-6 py-4 pl-12">{m.popularity.toFixed(2)}</td>
                ...
                <td className="px-6 py-4">
                  <span className="cursor-pointer text-red-600 font-semibold" onClick={()=>handleDelete(m.id)}>Delete</span>
                </td>
              </tr>
            ))}
          </tbody>
